...
if [ -e ".git" ]; then
	...
	git config --local gitflow.feature.finish.rebase true
	...
	git config --local alias.fbr '!f() { b=$(git branch-name); if [[ "$b" == feature* ]]; then echo "Rebasing $b to latest master commit";git checkout master; git pull origin master 
	--rebase; git checkout "$b"; git rebase master; echo "Rebasing done"; else echo "$b" is not a feature branch, nothing to do";fi;}; f'
	echo -e ""
